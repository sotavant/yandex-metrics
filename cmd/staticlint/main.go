...
const staticcheckClass = "SA"
const staticcheckAdditionalCheckCode = "S1005"
...
func getStaticcheckAnalyzers() []*analysis.Analyzer {
	...
	for _, v := range staticcheck.Analyzers {
		if strings.HasPrefix(v.Analyzer.Name, staticcheckClass) {
			checks = append(checks, v.Analyzer)
		} else if v.Analyzer.Name == staticcheckAdditionalCheckCode {
			checks = append(checks, v.Analyzer)
		}
	}
...
func getAdditionalAnalyzers() []*analysis.Analyzer {
	var checks []*analysis.Analyzer
	checks = append(checks, magic_numbers.Analyzer)
